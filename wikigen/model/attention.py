...
class ScoringLayer(nn.Module):
    ...
    def __init__(self, input_size, att_type='concat'):
        ...
        if self.att_type == 'concat':
            ...
            self.W = Parameter(torch.Tensor(2 * self.input_size,
                                            self.input_size))
            # self.context is w in the paper
            self.context = Parameter(torch.Tensor(self.input_size))

        elif self.att_type == 'general':
            self.W = Parameter(torch.Tensor(self.input_size,
                                            self.input_size))
        else:
            self.W = None
    ...
    def forward(self, hidden_states, sent_repr):
        ...
        if self.att_type == 'concat':
            ...
            context = self.context.unsqueeze(0).expand(batch_size,
                                                       hidden_x_dirs)
...
class AttentionLayer(nn.Module):
    ...
    def __init__(self, input_size, scoring_scheme='dot'):
        ...
        self.scoring_layer = ScoringLayer(self.input_size,
                                          att_type=scoring_scheme)
