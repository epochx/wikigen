...
def update_active(t, active_idx, num_active_beams, beam_size,
                  batch_beam_index=0):
...
def beam_search_decode(model,
    ...
    batch_size, src_seq_len = src_batch_sequences.size()

    # batch_size, seq_len, hidden_x_dirs
    h_t, context_batch = model.encode(src_batch_sequences,
                                      src_batch_lengths,
                                      src_batch_tag_sequences)
    ...
    beam_context_batch = beam_context_batch.view(batch_size*beam_size,
                                                 src_seq_len,
                                                 -1)
    ...
    beam_context_batch_mask = beam_context_batch_mask.view(batch_size*beam_size,
                                                           src_seq_len)

    # -> (1, batch_size*beam_size, hidden_size)
    beam_h_t = h_t.unsqueeze(1)
    beam_h_t = beam_h_t.repeat(1, beam_size, 1)
    beam_h_t = beam_h_t.view(1, batch_size*beam_size, -1)

    # -> (1, batch_size*beam_size, hidden_size)
    beam_c_t = h_t.unsqueeze(1)
    beam_c_t = beam_c_t.repeat(1, beam_size, 1)
    beam_c_t = beam_c_t.view(1, batch_size * beam_size, -1)

    beam_decoder_hidden_tuple_i = (beam_h_t, beam_c_t)

    beams = [Beam(beam_size,
                  vocab.tgt.PAD.hash,
                  vocab.tgt.BOS.hash,
                  vocab.tgt.EOS.hash) for k in range(batch_size)]
    ...
    for i in range(max_length):
        ...
        beam_tgt_batch_sequences_i = torch.stack([b.get_current_state()
                                                  for b in beams if not b.done])

        # -> (num_active*beam_size, 1)
        beam_tgt_batch_sequences_i = \
            beam_tgt_batch_sequences_i.view(num_active * beam_size, 1)

        beam_decoder_hidden_tuple_i, beam_logits_i = \
            model.generate(beam_tgt_batch_sequences_i,
                           beam_decoder_hidden_tuple_i,
                           beam_context_batch,
                           beam_context_batch_mask)

        beam_att_i = model.attention[-1]
        ...
        beam_logits_i = beam_logits_i.view(num_active,
                                           beam_size,
                                           -1).contiguous()

        # -> (num_active, beam_size, src_seq_len)
        beam_attentions_i = beam_att_i.view(num_active,
                                            beam_size,
                                            -1).contiguous()
        ...
        for b in range(batch_size):
            ...
            if not beams[b].advance(beam_logits_i[idx],
                                    beam_attentions_i[idx]):
                active += [b]
            ...
            for beam_decoder_hidden in beam_decoder_hidden_tuple_i:
                ...
                beam_decoder_hidden = beam_decoder_hidden.view(1,
                                                               num_active,
                                                               beam_size,
                                                               -1)
        ...
        beam_decoder_hidden_tuple_i = \
            (update_active(beam_decoder_hidden_tuple_i[0],
                           active_idx,
                           num_active,
                           beam_size,
                           batch_beam_index=1),
             update_active(beam_decoder_hidden_tuple_i[1],
                           active_idx,
                           num_active,
                           beam_size,
                           batch_beam_index=1))
        ...
        beam_context_batch = update_active(beam_context_batch,
                                           active_idx,
                                           num_active,
                                           beam_size,
                                           batch_beam_index=0)
        ...
        # -> (num_active*beam_size, src_seq_len)
        beam_context_batch_mask = update_active(beam_context_batch_mask,
                                                active_idx,
                                                num_active,
                                                beam_size,
                                                batch_beam_index=0)
    ...
    return allHyp, allAttn


def beam_search_pointer_generator(model,
                                  vocab,
                                  src_batch_tuple,
                                  batch_oovs,
                                  max_length,
    ...
    batch_oov_count = len(batch_oovs)
    ...
    # -> (batch_size*beam_size, src_seq_len)
    beam_src_batch_sequences = src_batch_sequences.unsqueeze(1)
    beam_src_batch_sequences = beam_src_batch_sequences.repeat(1, beam_size, 1)
    beam_src_batch_sequences = beam_src_batch_sequences.view(batch_size*beam_size,
                                                             src_seq_len)
    ...
    for i in range(max_length):
        ...
        beam_decoder_hidden_tuple_i, beam_logits_i = \
            model.generate(beam_tgt_batch_sequences_i,
                           beam_decoder_hidden_tuple_i,
                           beam_context_batch,
                           beam_src_batch_sequences,
                           beam_context_batch_mask,
                           batch_oov_count)

        beam_att_i = model.attention[-1]

        beam_p_gen_i = model.p_gen[-1]

        # (num_active*beam_size, output_size)
        beam_logits_i = beam_logits_i.log()
        ...
        beam_p_gen_i = beam_p_gen_i.view(num_active,
                                         beam_size,
                                         -1).contiguous()
        ...
        beam_src_batch_sequences = update_active(beam_src_batch_sequences,
                                                 active_idx,
                                                 num_active,
                                                 beam_size,
                                                 batch_beam_index=0)
    ...
    # (4) package everything up
    n_best = 1
    allHyp, allScores, allAttn, allPgen = [], [], [], []
