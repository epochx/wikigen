...
def get_embedding_matrix(embeddings, vocab):
    ...
    matrix = torch.randn((vocab_size , embeddings.vector_size),
                          dtype=torch.float32)

    for token, idx in vocab.token2index.items():
        if token in embeddings.wv:
            matrix[idx] = torch.from_numpy(embeddings.wv[token])
...
def feed_forward_rnn(rnn,
    ...
    if lengths is not None:
        rnn_input, indices_unsort = \
            pack_rnn_input(embedded_sequence_batch, lengths)
